...
class ViewController: BaseViewController, SCNSceneRendererDelegate {
    ...
    private enum CollectableState: Int {
        case notCollected = 0
    ...
    */
    ...
    required init?(coder: NSCoder) {
        ...
        character.add(idleAnimation, forKey: "start")
        ...
        wheels.add(wheelAnimation, forKey: "wheel");
    ...
    override func viewDidLoad() {
        ...
        scene.rootNode.add(cartAnimation, forKey: cartAnimationName)
    ...
    func triggerCollision() {
        ...
        character.run(.play(hitSound, waitForCompletion: false))
        character.add(slapAnimation, forKey: nil)
    ...
    private func collectItems() {
        ...
        for collectable in collectables.childNodes {
            guard collectable.categoryBitMask != CollectableState.beingCollected.rawValue else { continue }
            ...
            if length_squared(leftHandPosition - collectablePosition) < 0.05 || length_squared(rightHandPosition - collectablePosition) < 0.05 {
                collectable.categoryBitMask = CollectableState.beingCollected.rawValue
                ...
                #if os(iOS) || os(tvOS)
                scene.addParticleSystem(collectParticleSystem, withTransform: collectable.presentation.worldTransform)
                #else
                scene.addParticleSystem(collectParticleSystem, with: collectable.presentation.worldTransform)
                #endif

                if let name = collectable.name where name.hasPrefix("big") {
                    sceneView.didCollectBigItem()
                    collectable.run(.play(collectSound2, waitForCompletion: false))
                }
                else {
                    sceneView.didCollectItem()
                    collectable.run(.play(collectSound, waitForCompletion: false))
                }
        ...
        for collectable in speedItems.childNodes {
            ...
            if length_squared(rightHandPosition - collectablePosition) < 0.05 {
                ...
                pov.run(.sequence([.wait(forDuration: 2.0), adjustCamera]))
                character.run(.play(cartBoost, waitForCompletion: false))
    ...
    func updateSpeed () {
        ...
        scene.rootNode.setSpeed(effectiveSpeed, forAnimationKey: cartAnimationName)
        wheels.setSpeed(effectiveSpeed, forAnimationKey: "wheel")
        idleAnimationOwner.setSpeed(effectiveSpeed, forAnimationKey: "bob_idle-1")
    ...
    func squat() {
        ...
        character.add(squatAnimation, forKey: nil)
        character.run(.play(cartHide, waitForCompletion: false))
        
        SCNTransaction.commit()
    }
    
    func jump() {
        character.add(jumpAnimation, forKey: nil)
        character.run(.play(cartJump, waitForCompletion: false))
    }
    
    func leanLeft() {
        character.add(leanLeftAnimation, forKey: nil)
        character.run(.play(cartTurnLeft, waitForCompletion: false))
    }
    
    func leanRight() {
        character.add(leanRightAnimation, forKey: nil)
        character.run(.play(cartTurnRight, waitForCompletion: false))
    }
    ...
    func startMusic() {
        ...
        sceneView.scene?.rootNode.run(.play(musicIntroSource, waitForCompletion: true)) { [unowned self] in
    ...
    func startGameIfNeeded() -> Bool {
        ...
        character.add(idleAnimation, forKey: nil)
