...
var canonkey;
(function (canonkey) {
    var Constraints_html = /** @class */ (function (_super) {
        ...
        Constraints_html.prototype.spherePendulum = function () {
            ...
            var $disLock = new canonkey.CanonPrefabSprite(spherebody2);
            $disLock.addToWorld();
        ...
        Constraints_html.prototype.lockScene = function () {
            canonkey.Physics.world.gravity.set(0, 0, -10);
            var size = 0.5;
            var mass = 1;
            var space = 0.1 * size;
            var N = 10;
            ...
            for (var i = 0; i < N; i++) {
                ...
                var $disLock = new canonkey.CanonPrefabSprite($bodyLock);
                $disLock.addToWorld();
                if (last) {
                    // Connect the current body to the last one
                    var c = new CANNON.LockConstraint($bodyLock, last);
                    canonkey.Physics.world.addConstraint(c);
                }
                // To keep track of which body was added last
                last = $bodyLock;
            }
