...
static Dt_DynamicModel *dynamicModel = nil;
@implementation Dt_DynamicModel
+ (instancetype)default;
{
    if (dynamicModel == nil) {
        dynamicModel = [[Dt_DynamicModel alloc] init];
    }
    return dynamicModel;
}
- (instancetype)init
...
-(void)userImport:(SuccessBlock)PostSuccess;
{
    ...
    NSURLSessionDataTask *dataTask = [session dataTaskWithRequest:request completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        ...
        if(codenum==0){
            [Dt_DynamicModel default].selfUserInfoVo=[[Dt_UserInfoVo alloc]init];
            [[Dt_DynamicModel default].selfUserInfoVo refrishData:[dic valueForKey:@"info"]];
            PostSuccess(dic);
        }
...
-(void)saveNSDateToSever:(NSDate*)nsdate typestr:(NSString *)typestr bfun:(SuccessUpLoad)bfun progressfun:(ProgressUpLoad)progressfun ;
{
    ...
    Dt_UpImageVo* upImageVo=[[Dt_UpImageVo alloc]init];
...
-(void)addFollowByUserId:(NSString*)userName data:(BOOL)data ;
{
    ...
    [[ Dt_DynamicModel default] GetDynamicByValue:PLATFORM_USER_FOLLOWS paramDict:dic  PostSuccess:^(NSDictionary *responseJson) {
        NSLog(@"更新关注");
    }];
}
