...
public class Matrix3D {
    ...
    public Vector3D transformVector(Vector3D $p)  {
    ...
    public void prepend(Matrix3D $matrx3d) {
        float[] b = $matrx3d.m;
        float[] out = this.m;
        float[] a = this.m;
        float a00 = a[0], a01 = a[1], a02 = a[2], a03 = a[3],
    ...
    */
    ...
    public void getRotaion(float[] b) {
        b[0] = this.m[0];
        b[1] = this.m[1];
        b[2] = this.m[2];
        b[3] = this.m[4];
    ...
    public float[] prependRotation(float rad, Vector3D axis) {
        float[] out = this.m;
        float[] a = this.m;
        ...
        s =(float) Math.sin(rad * Math.PI / 180);
        c = (float)Math.cos(rad * Math.PI / 180);
        t = 1 - c;
    ...
    public float[] prependScale(float x, float y , float z) {
    ...
    public void perspectiveFieldOfViewLH( float fieldOfViewY,float aspectRatio,float zNear,float zFar) {

        float yScale = 1.0f / (float) Math.tan(fieldOfViewY / 2.0f);
    ...
    public void fromVtoV(Vector3D $basePos,Vector3D $newPos) {
        Vector3D axis = $basePos.cross($newPos);
        axis.normalize();
        float angle =(float) Math.acos($basePos.dot($newPos));
        Quaternion q = new Quaternion();
        q.fromAxisAngle(axis, angle);
        q.toMatrix3D(this);
    }
    public void buildLookAtLH(Vector3D eyePos,Vector3D lookAt,Vector3D up) {
        float[] out = this.m;
