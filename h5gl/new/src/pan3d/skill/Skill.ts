module Pan3d {
    export class Skill extends ResCount {
        ...
        public actionEnd: boolean = false
        ...
        public setKeyAry(): void {
            ...
            if (this.skillVo.types == SkillType.FixEffect) {
                for (var i: number = 0; i < this.skillVo.keyAry.length; i++) {
                    var keySkill: SkillFixEffect = new SkillFixEffect(this.scene3D);
                ...
                for (var i: number = 0; i < this.skillVo.keyAry.length; i++) {
                    ...
                    if (tkv.multype == 1) {
                        trajectory = new SkillMulTrajectory(this.scene3D);
                    } else {
                        trajectory = new SkillTrajectory(this.scene3D);
                    }
        ...
        public setendParticleRoation($vect3d: Vector3D): void {
            for (var i: number = 0; this.keyAry&& i < this.keyAry.length; i++) {
                if (this.keyAry[i] instanceof SkillTrajectory) {
                    if ((<SkillTrajectory>this.keyAry[i]).endParticle) {
                        (<SkillTrajectory>this.keyAry[i]).endParticle.rotationX = $vect3d.x;
                        (<SkillTrajectory>this.keyAry[i]).endParticle.rotationY = $vect3d.y;
                        (<SkillTrajectory>this.keyAry[i]).endParticle.rotationZ = $vect3d.z;
                    }
                }
            }
        }
