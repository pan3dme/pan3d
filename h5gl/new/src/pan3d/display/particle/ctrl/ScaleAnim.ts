module Pan3d {
    export class ScaleAnim extends BaseAnim {
        ...
        public coreCalculate(): void {
            var frameNum: number = float2int(this.time / Scene3D.frameTime);
        ...
        public set data(value: Array<any>) {
            ...
            if (Number(value[1].value) == -1) {
                this.lastTime = Scene3D.MAX_NUMBER;
            } else {
            ...
            if (this.scaleAry.length) {
                frameNum = (this.scaleAry[this.scaleAry.length - 1].beginTime + this.scaleAry[this.scaleAry.length - 1].time) / Scene3D.frameTime;
            ...
            for (i = 0; i < frameNum; i++) {
                var ctime: number = Scene3D.frameTime * i;
