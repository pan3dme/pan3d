...
module Pan3d {
    ...
    export  class Md5MeshSprite extends Display3DSprite {
        ...
        private loadBodyMesh($url: string): void {
            LoadManager.getInstance().load(this.scene3D.fileRoot + $url, LoadManager.XML_TYPE, ($str: any) => {
                var $md5Analysis: Md5Analysis = new Md5Analysis(this.scene3D)
                this._md5MeshData = $md5Analysis.addMesh($str);
                new MeshImportSort(this.scene3D).processMesh(this._md5MeshData);
